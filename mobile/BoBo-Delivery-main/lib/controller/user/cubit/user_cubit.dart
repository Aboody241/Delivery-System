...
class UserCubit extends Cubit<UserState> {
  ...
  Future<void> fetchUser() async {
    ...
    final uid = await _authService.getCurrentUserUid() ?? 'guest';
    final email = await _authService.getCurrentUserEmail() ?? 'user@example.com';

    emit(UserLoading());

    try {
      final userModel = await _userRepository.getUser(uid, email);
